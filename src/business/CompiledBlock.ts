...
const MetaSchema = z.object({
    ...
    format_version: z
        .number()
        .min(currentFormatVersion)
        .max(currentFormatVersion)
        .default(currentFormatVersion),
});
...
const EventsSchema = z.object({
    on_break: BreakEventSchema.default({}),
});
    ...
    symbols: z
        .array(
            z.record(
                z.string(),
                z
                    .union([z.enum(MinecraftBlocks), z.enum(MinecraftItems)])
                    .default(MinecraftBlocks.STONE)
            )
        )
        .default([{ X: MinecraftBlocks.STONE }]),
});
...
const RecipeSchema = z.object({
    ...
    workbench: WorkbenchSchema.default({}),
    furnace: FurnaceSchema.default({}),
});
